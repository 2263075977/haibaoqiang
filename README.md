...
### Chrome和ChromeDriver问题

如果遇到以下错误：

```
初始化Chrome浏览器失败: 多次尝试初始化浏览器失败: There is no such driver by url https://chromedriver.storage.googleapis.com/LATEST_RELEASE_XXX
```
...
1. **运行健康检查脚本**:
   ```bash
   docker exec douban-notion python healthcheck.py
   ```
   这将显示Chrome和ChromeDriver的详细信息，帮助诊断问题。
...
4. **ARM设备注意事项**:
   在树莓派等ARM设备上，应使用`Dockerfile.arm64`构建镜像，它会使用Chromium而不是Chrome。
...
1. **检查网络连接**
2. **配置豆瓣Cookie**:
   在`.env`文件中设置豆瓣Cookie可以提高访问成功率。
3. **调整重试次数**:
   修改`douban_crawler.py`中的`max_retries`值以增加重试次数。

### 内存不足问题

Chrome在容器中可能会消耗大量内存。如果遇到OOM（内存不足）错误：

1. **增加共享内存**:
   修改`docker-compose.yml`中的`shm_size`：
   ```yaml
   shm_size: 4gb  # 增加到4GB或更多
   ```

2. **限制并发任务数**:
   API服务可能同时处理多个请求，如果内存有限，考虑设置并发限制。 